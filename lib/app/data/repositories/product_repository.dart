...
class ProductRepository {
  ...
  Future<Either<ProductModel, Failure>> listProduct() async {
    try {
  ...
  Future<Either<bool, Failure>> deleteProduct(String urlSlug) async {
    try {
      var response = await dio.delete(ApiConstants.DELETEPRODUCT + '$urlSlug/');
  ...
  Future<Either<ProductModel, Failure>> searchProduct(String query) async {
    try {
      var response = await dio.get(ApiConstants.SEARCHPRODUCT + '$query/');
      stdout.write(response.data);
      if (response.statusCode == 201 || response.statusCode == 200) {
        return Left(productModelFromMap(jsonEncode(response.data)));
      }
